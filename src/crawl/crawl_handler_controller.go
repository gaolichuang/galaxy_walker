...
type CrawlHandlerController struct {
    InputProcessors []CrawlTask
    ProcessChain    []CrawlTask
    inited          bool
}
...
func (c *CrawlHandlerController) startServiceInterval() {
    for _, p := range c.ProcessChain {
        ...
        CHECK(p.Init(), "%s Init Fail", reflect.TypeOf(p))
        go p.Run(p.(CrawlProcessor))
    }
    for _, r := range c.InputProcessors {
        LOG.Infof("%s Start to Run", reflect.TypeOf(r))
        CHECK(r.Init(), "%s Init Fail", reflect.TypeOf(r))
        go r.Run(r.(CrawlProcessor))
    }
}
func (c *CrawlHandlerController) InitCrawlService() {
    if c.inited == true {
        CHECK(false, "Handler Controller should only Init Once.")
    }
    c.inited = true
