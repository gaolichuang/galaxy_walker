...
func (cf *CrawlerFeeder) Connect() bool {
    if !cf.connected {
        ...
        opts = append(opts, grpc.WithTimeout(time.Second*time.Duration(*CONF.Crawler.RpcConnectionTimeOut)))
...
func (d *Dispatcher) LoadCrawlersFromFile(name string) {
    file.FileLineReader(name, "#", func(line string) {
        ...
        CHECK(len(addr) == 2, "Parse Addr Fail. %s", line)
        addrPort, err := strconv.Atoi(addr[1])
        CHECK(addrPort != *CONF.Crawler.DispatcherPort, "Dispatch itself, %s:%d", addr[0], addrPort)
        CHECK(err == nil, "atoi error %s", err)
